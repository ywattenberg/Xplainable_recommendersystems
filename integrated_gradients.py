...
def main():
    ...
    white_base_img = image_transform(Image.fromarray(np.ones([3,224,224], dtype=np.uint8))).to(device).unsqueeze(0)
    black_base_img = image_transform(Image.fromarray(np.zeros([3,224,224], dtype=np.uint8))).to(device).unsqueeze(0)
